...
#include "kprocessrunnerfactory.h"

static int s_instanceCount = 0; // for the unittest
static QScopedPointer<KProcessRunnerFactory> s_factory;

void KProcessRunner::setProcessRunnerFactory(KProcessRunnerFactory *factory)
{
    s_factory.reset(factory);
}
...
static KProcessRunner *makeInstance(KProcessRunnerFactory *factory = Q_NULLPTR)
{
    if (factory)
    {
        auto *processRunner = factory->create();
        if (processRunner)
        {
...
{
    auto instance = makeInstance(s_factory.get());
